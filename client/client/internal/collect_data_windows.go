...
func CollectData() (MachineInfo, error) {
	...
	if err != nil {
		Log.Errorf("Failed to collect MACs: %v", err)
	...
	if err != nil {
		Log.Warnf("Failed to collect network interfaces: %v", err)
	} else {
	...
	info.ComputerModel = computerModelWindows()
	info.ComputerActivation = computerActivationWindows()
