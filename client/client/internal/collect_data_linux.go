...
func isVirtualLinuxBySysfs(name string) bool {
	...
	// Fallback to name patterns when sysfs is not accessible.
...
func CollectData() (MachineInfo, error) {
	...
	if err != nil {
		Log.Warnf("Failed to collect hostname: %v", err)
	...
	if err != nil {
		Log.Errorf("Failed to collect network interfaces: %v", err)
	...
	for _, iface := range interfaces {
		...
		if err != nil {
			Log.Errorf("Failed to collect addresses from interface %s: %v", iface.Name, err)
			continue
		}
		hasValidIP := false
	...
	info.ComputerModel = computerModelLinux()
	info.ComputerActivation = computerActivationLinux()
	Log.Debugf("Computer model: %q, activation: %q", info.ComputerModel, info.ComputerActivation)
