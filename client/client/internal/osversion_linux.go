...
func getOSVersionLinux() string {
	...
	if err == nil {
		...
		Log.Debugf("cannot read /etc/os-release: %v", err)
	}
	...
	if err == nil {
		...
		Log.Debugf("cannot read /etc/lsb-release: %v", err)
	}
	...
	if err == nil {
		...
		Log.Debugf("cannot read /etc/redhat-release: %v", err)
	}
	...
	if err != nil {
		Log.Warnf("Failed to run uname: %v", err)
